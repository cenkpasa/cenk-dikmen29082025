...
import { Customer, Invoice, Offer, IncomingInvoice, OutgoingInvoice, Warehouse, StockItem, StockLevel, MileageReportData, FinancialData } from '../types';
import { MOCK_OUTGOING_INVOICES, MOCK_INCOMING_INVOICES, CSV_DATA, MOCK_ERP_OFFERS, MOCK_ERP_STOCK_ITEMS, MOCK_ERP_WAREHOUSES, MOCK_ERP_STOCK_LEVELS, MOCK_MILEAGE_REPORT_DATA, MOCK_PROFIT_LOSS_DATA } from './erpMockData';
...
export const fetchIncomingInvoices = (): Promise<IncomingInvoice[]> => {
    ...
    return new Promise(resolve => {
        setTimeout(() => {
            resolve(MOCK_INCOMING_INVOICES.map(inv => ({...inv, id: inv.faturaNo})));
        }, SIMULATED_DELAY);
    });
};
...
export const fetchOutgoingInvoices = (): Promise<OutgoingInvoice[]> => {
    ...
    return new Promise(resolve => {
        setTimeout(() => {
            resolve(MOCK_OUTGOING_INVOICES.map(inv => ({...inv, id: inv.faturaNo})));
        }, SIMULATED_DELAY);
    });
};
...
export const fetchMileageReportData = (userId: string, periodStart: string, periodEnd: string): Promise<MileageReportData | null> => {
    console.log(`Simulating ERP fetch for MILEAGE report for user ${userId}...`);
    return new Promise(resolve => {
        setTimeout(() => {
            // In a real app, you'd filter logs by date here.
            resolve(MOCK_MILEAGE_REPORT_DATA);
        }, SIMULATED_DELAY);
    });
};

export const fetchProfitLossData = (year: number): Promise<FinancialData | null> => {
    console.log(`Simulating ERP fetch for P&L statement for year ${year}...`);
     return new Promise(resolve => {
        setTimeout(() => {
            resolve(MOCK_PROFIT_LOSS_DATA);
        }, SIMULATED_DELAY);
    });
};
...
const parseErpData = (csvText: string): ParsedResult => {
    ...
    for (const line of lines) {
        ...
        if (!customers.has(currentCode)) {
             // Fix: Added required 'registrationDate' to satisfy Omit<Customer, 'id' | 'createdAt'> type
